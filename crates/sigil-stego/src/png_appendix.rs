...
const SIGIL_MAGIC: &[u8] = b"SIGILV1\0";
...
/// ignore it; Sigil reads it by scanning from the end.
pub fn write_vault_to_png(png_path: &Path, vault_bytes: &[u8]) -> anyhow::Result<()> {
    ...
    // Append: SIGIL_MAGIC || vault_length_u64_le || vault_bytes
    let vault_len = vault_bytes.len() as u64;
    png_data.extend_from_slice(SIGIL_MAGIC);
...
/// Extract vault bytes from a PNG with a Sigil appendix.
...
pub fn extract_from_bytes(data: &[u8]) -> anyhow::Result<Vec<u8>> {
    // Scan for SIGIL_MAGIC from the end (efficient for large images).
    let magic_len = SIGIL_MAGIC.len();
    let min_len = magic_len + 8; // magic + u64 length

    if data.len() < min_len {
        anyhow::bail!("no Sigil appendix found");
    }

    // Search for SIGIL_MAGIC.
    let search_start = data.len().saturating_sub(data.len()); // search whole file
    let pos = data
        .windows(magic_len)
        .rposition(|w| w == SIGIL_MAGIC)
        .ok_or_else(|| anyhow::anyhow!("no Sigil appendix found in file"))?;

    let after_magic = pos + magic_len;
    if after_magic + 8 > data.len() {
        anyhow::bail!("truncated Sigil appendix");
    }
    ...
    if vault_start + vault_len > data.len() {
        anyhow::bail!("Sigil appendix length exceeds file size");
    }
...
mod tests {
    ...
    fn appendix_roundtrip() {
        ...
        // Simulate write.
        data.extend_from_slice(SIGIL_MAGIC);
